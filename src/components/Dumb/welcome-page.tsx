...
const styles = {
  ...
  button: "text-gray-300 rounded-md text-base from-purple-500 to-violet-800 dark:text-gray-800 dark:from-teal-200 dark:to-sky-600 hover:cursor-pointer font-semibold font-stretch-expanded bg-gradient-to-br hover:bg-gradient-to-bl focus:ring-3 focus:outline-none focus:ring-violet-900 dark:focus:ring-sky-100 font-medium text-md px-4 py-2 text-center flex items-center justify-center w-[9vw] h-[4vh] min-w-[150px] min-h-[40px]",
  smallText: "absolute bottom-0 p-[5] text-center text-xs",
  input: "p-2 bg-slate-300 dark:bg-slate-600 rounded-md border-2 border-gray-500 w-1/2 text-center focus:ring-3 focus:outline-none focus:ring-violet-900 dark:focus:ring-sky-100",
  btnContainer: "flex space-x-4"
};

export function Landing(props: { onClick: () => void }) {
  return (
    <div className={styles.centerPanel}>
      ...
      <button className={styles.button} onClick={props.onClick}>Load Existing</button>
      <p className={styles.smallText}>© {new Date().getFullYear()} Rachel Lewis</p>
    </div>
  );
}

export function LoadPortfolio(props: { onClick: () => void }) {
  return (
    <div className={styles.centerPanel}>
      <input type='text' placeholder='Portfolio ID' className={styles.input} />
      <div className={styles.btnContainer}>
        <button className={styles.button}>Load Portfolio</button>
        <button className={styles.button} onClick={props.onClick}>Back</button>
      </div>
      <p className={styles.smallText}>© {new Date().getFullYear()} Rachel Lewis</p>
    </div>
  );
